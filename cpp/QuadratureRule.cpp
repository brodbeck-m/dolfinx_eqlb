...
QuadratureRuleNew::QuadratureRuleNew(dolfinx::mesh::CellType ct, int degree,
    ...
    : _cell_type(ct), _degree(degree), _type(type), _dim(dim)
{
  ...
  if (_tdim == std::size_t(dim))
  {
    ...
    dolfinx_adaptivity::mdspan_t<const double, 2> qp(quadrature.front().data(),
                                                     num_points, pt_shape);
...
//-----------------------------------------------------------------------------------------------
dolfinx::mesh::CellType QuadratureRuleNew::cell_type(std::int8_t i) const
{
...
//-----------------------------------------------------------------------------------------------
int QuadratureRuleNew::degree() const { return _degree; }
//-----------------------------------------------------------------------------------------------
basix::quadrature::type QuadratureRuleNew::type() const { return _type; }
//-----------------------------------------------------------------------------------------------
std::size_t QuadratureRuleNew::num_points(std::int8_t i) const
{
  assert(i < _num_sub_entities);
...
QuadratureRuleNew::points(std::int8_t i) const
{
  assert(i < _num_sub_entities);
...
//-----------------------------------------------------------------------------------------------
std::span<const double> QuadratureRuleNew::weights(std::int8_t i) const
{
  assert(i < _num_sub_entities);
